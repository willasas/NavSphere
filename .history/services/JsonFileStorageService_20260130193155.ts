...
export class JsonFileStorageService {
  ...
  constructor(storagePath: string = path.join(process.cwd(), 'navsphere', 'content')) {
  ...
  private async initializeStorage() {
    try {
      ...
      // 确保存储文件存在
      const storageFilePath = path.join(this.storagePath, 'storage.json');
      try {
        ...
        await fs.writeFile(storageFilePath, JSON.stringify(defaultData, null, 2));
      }
    } catch (error) {
