// 检测是否在Edge运行时中
const isEdgeRuntime = typeof process === 'undefined' || process.env.NODE_ENV === 'production';
...
export class JsonFileStorageService {
  ...
  private supportsFileSystem(): boolean {
    return !isEdgeRuntime && this.fs && this.path;
  }
  ...
  private async initializeStorage() {
    ...
    try {
      // 确保存储目录存在
      const fullStoragePath = this.path.join(process.cwd(), this.storagePath);
      await this.fs.mkdir(fullStoragePath, { recursive: true });

      // 确保存储文件存在
      const storageFilePath = this.path.join(fullStoragePath, 'storage.json');
      try {
        await this.fs.access(storageFilePath);
      } catch {
        // 创建默认存储文件
        ...
        await this.fs.writeFile(storageFilePath, JSON.stringify(defaultData, null, 2));
      }
    } catch (error) {
  ...
  private async readStorage(): Promise<StorageData> {
    ...
    const storageFilePath = this.path.join(process.cwd(), this.storagePath, 'storage.json');
    try {
      const data = await this.fs.readFile(storageFilePath, 'utf8');
  ...
  private async writeStorage(data: StorageData): Promise<void> {
    ...
    const storageFilePath = this.path.join(process.cwd(), this.storagePath, 'storage.json');
    try {
      await this.fs.writeFile(storageFilePath, JSON.stringify(data, null, 2));
      // 清除缓存
      this.cache.clear();
    } catch (error) {
      console.error('写入存储失败:', error);
      // 在生产环境中，我们不应该因为存储写入失败而中断操作
  ...
  async deleteResourceMetadata(hashes: string[]) {
    try {
      ...
      // 过滤掉要删除的项
      storageData.resource_metadata = storageData.resource_metadata.filter(
        (item: any) => !hashes.includes(item.id)
      );

      await this.writeStorage(storageData);
      const deletedCount = originalLength - storageData.resource_metadata.length;
